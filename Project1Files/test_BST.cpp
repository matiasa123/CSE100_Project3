...
int main() {
  ...
  for(; vit != ven; ++vit) {
    ...
    std::pair<BST<int>::iterator,bool> pr = b.insert(*vit);
      if( pr.second ) {
      cout << "Correct bool return value when inserting " << *vit << endl;
      }
      if(! pr.second ) {
  ...
  cout << "Testing find return value: " << endl;
  vit = v.begin();
  for(; vit != ven; ++vit) {
    if(*(b.find(*vit)) != *vit) {
  ...
  cout << "Sorting the vector, to compare with inorder iteration on the BST" << endl;
  sort(v.begin(),v.end());

  /* Test BST iterator; should iterate inorder */
  cout << "now testing the BST iterator; should iterate inorder:" << endl;
  cout << "traversal using iterator:" << endl;
  vit = v.begin();
  BST<int>::iterator en = b.end();
  BST<int>::iterator it = b.begin();
  
  cout << "now checking correctedness of iterator: " << endl;
  for(; vit != ven; ++vit) {
    if(! (it != en) ) {
